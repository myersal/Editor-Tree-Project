...
//	}
	...
	public String get(int start, int end) {
		...
		//if teh index is within range it recurses both ways
		if(start <= this.rank && end >= this.rank) {
			return this.left.get(start, end) + this.element + this.right.get(start - this.rank, end - (this.rank + 1));
		}
		// if the starting index is greater than the rank it recurses only to the right
		if(start > this.rank) {
			return this.right.get(start - (this.rank + 1), end - (this.rank + 1));
		}
		//if the ending index is less then the rank it recurses only left
		if(end < this.rank) {
