...
public class EditTree {
		...
		while (current.parent != EditTree.NULL_NODE) { // makes sure the current
			...
			if (current.parent.right.equals(current)) {
				...
				if (current.parent.balance == Code.RIGHT) {
					...
					// if the current is balanced or right and parent is right
					// it is a
					// single rotation
					Node temp = root
							.singleLeftRotation(current.parent, current);
					if (temp.parent == NULL_NODE) {
						root = temp;
					}
	...
	public char delete(int pos) throws IndexOutOfBoundsException {
		...
		rotationAfterDelete(direction, prevNode);
		return returnChar;
	}
	
	/**
	 * 
	 * Helper method for delete to rotate the tree.
	 *
	 * @param direction
	 * 		previous direction that the method ran
	 * @param prevNode
	 * 		previous node to check rotations on
	 */
	
	public void rotationAfterDelete(Code direction, Node prevNode) {
		...
		while (prevNode != NULL_NODE) { // traverse back up to check balance
			...
			if (direction == prevNode.balance) {
				...
				if (direction == Code.RIGHT) {
					...
					// checks the ambiguous case for when the last direction was
					// left
					else if (rotaedNode.balance == Code.LEFT
							&& rotaedNode.left.balance == Code.SAME) {
						// does the single rotation and then balance codes need
						// to be redone due to the ambiguous case
						rotaedNode = root.singleRightRotation(rotaedNode,
								rotaedNode.left);
						rotationCounter++;
						// fixes balance codes
						rotaedNode.balance = Code.RIGHT;
						rotaedNode.right.balance = Code.LEFT;
						// breaks since the larger tree's height hasn't been
						// changed
						break;
					}
					...
					if (rotaedNode.balance == Code.RIGHT
						...
						rotationCounter += 2;
						// checks the ambiguous case
					} else if (rotaedNode.balance == Code.RIGHT
							&& rotaedNode.left.balance == Code.SAME) {
						// does the rotation and then fixes balance codes
						rotaedNode = root.singleRightRotation(rotaedNode,
								rotaedNode.left);
						rotationCounter++;
						// fixes balance codes since this is a special case
						rotaedNode.balance = Code.LEFT;
						rotaedNode.right.balance = Code.RIGHT;
						// breaks since the larger tree's height hasn't been
						// changed
						break;
	...
	public String get(int pos, int length) throws IndexOutOfBoundsException {
		//sets the end index that for the String
		int end = pos + length - 1;
		//checks to see if the indexes are out of range
		if (end > this.size() - 1 || pos < 0 || pos > end) {
			throw new IndexOutOfBoundsException();
		}
	...
	public void concatenate(EditTree other) throws IllegalArgumentException {
		...
		// checks to see if the other tree is empty
		...
		if (other.height() < this.height()) {
			...
			for (int i = 0; i < heightDifference - 1; i++) {
			...
			//special case if the trees are equal in height then you make the splitter the new root
			if (heightDifference == 0) {
				//updates parent nodes and children then updates balance and rank
				splitter.parent = NULL_NODE;
				splitter.left = this.root;
				this.root.parent = splitter;
				splitter.right = other.root;
				other.root.parent = splitter;
				splitter.rank = this.root.size();
				this.root = splitter;
				this.root.balance = Code.SAME;
				other.root = NULL_NODE;
				return;
			}
			//goes through and finds the node to split at
			for (int i = 0; i < heightDifference; i++) {
				if (toSplitAt.balance == Code.RIGHT) {
					toSplitAt = toSplitAt.left.left;
				} 
				else if(toSplitAt.left != NULL_NODE) {
	...
	public EditTree split(int pos) throws IndexOutOfBoundsException {
		//quick and dirty to get the points if can't figure out efficient solution.
		...
		String leftString = tempString.substring(0, pos);
		String rightString = tempString.substring(pos);
		
		EditTree leftTree = new EditTree(leftString);
		EditTree rightTree = new EditTree(rightString);
...
//		int currentPos = root.rank;
//		Stack<Node> nodeStack = new Stack<Node>();
//		
//		if(root == NULL_NODE)
//			throw new IndexOutOfBoundsException();
//		//throw exceptions if the arg passed doesn't apply to the tree
//		if(pos < 0 || pos > currentNode.size())
//			throw new IndexOutOfBoundsException();
//		
//		//simple O(log n) loop to get to our desired pos
//		while(currentPos != pos){
//			nodeStack.push(currentNode);
//			if(currentPos < pos){
//				currentNode = currentNode.right;
//				currentPos = currentNode.rank + currentPos + 1;
//			}else if(currentPos > pos){
//				currentNode = currentNode.left;
//				currentPos = currentNode.rank;
//			}
//		}
...
//				tempTree.root = child;
//				rightTree.concatenate(tempTree);
//			}
//			
//			tempNode = nodeStack.pop();
//			if(tempNode.right.equals(child)){
//				EditTree tempTree = new EditTree();
